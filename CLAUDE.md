...
- **Apple** — `expo-apple-authentication`, `signInAsync` puis `signInWithIdToken({provider:'apple'})`. Le NOM n'est fourni qu'à la **toute première** connexion, jamais ensuite — recopié dans `profiles` immédiatement, sinon perdu pour de bon. `usesAppleSignIn: true` dans `app.json` (entitlement Apple — nécessite un build **interactif**, voir plus bas).
- **Google** — `@react-native-google-signin/google-signin`, `signInWithIdToken({provider:'google'})`. ⚠️ **« Skip nonce check » doit être activé** côté *Authentication → Providers → Google* : les SDK natifs mobiles ne savent pas satisfaire le nonce que Supabase attend par défaut (documenté officiellement par Supabase comme la solution standard pour iOS natif, pas un contournement). Deux Client IDs distincts : web (`EXPO_PUBLIC_GOOGLE_WEB_CLIENT_ID`, existant) et **iOS** (`EXPO_PUBLIC_GOOGLE_IOS_CLIENT_ID`, un type d'identifiant OAuth différent, à créer séparément dans Google Cloud) — les deux doivent figurer dans la liste *Client IDs* de Supabase.
- **Facebook** — `react-native-fbsdk-next`, `LoginManager.logInWithPermissions()` puis `signInWithIdToken({provider:'facebook'})` avec le jeton d'accès classique (pas un JWT — Supabase interroge l'API Graph côté serveur avec l'App Secret ; son guide Facebook ne documente que ce jeton-là, pas le Limited Login). ⚠️ **La librairie doit rester figée à `12.2.0` (`--save-exact`), jamais `13.x`.** À partir de la 13.0.0, elle adopte le SDK iOS natif de Meta v17 et **impose** le mode iOS « Limited Login » (repérable à l'écran par l'URL `limited.facebook.com`) sans exception possible — même `loginTrackingIOS: 'enabled'` passé explicitement n'a alors plus aucun effet. Limited Login renvoie un JWT qu'`AccessToken.getCurrentAccessToken()` ne récupère jamais et que Supabase ne sait pas valider : **le SDK réussit tout son échange avec Facebook, mais aucune identité n'est jamais créée côté Supabase, sans erreur claire.** 12.2.0 est le dernier choix sûr : dernière version avant ce changement, et première à supporter la New Architecture RN (donc aucun recul de compatibilité). Piège trouvé en confirmant via `auth.identities` qu'aucune ligne `provider='facebook'` n'existait malgré un flux qui semblait aboutir à l'écran ; `npx expo install`/`npm update` réinstalleraient la dernière version et referaient régresser le bug silencieusement. Côté Meta, la permission `email` doit être ajoutée dans **Use Cases → Authentication and Account Creation** (écran distinct de la config du produit Facebook Login) ; « Allow users without an email » activé côté Supabase en filet de sécurité.
