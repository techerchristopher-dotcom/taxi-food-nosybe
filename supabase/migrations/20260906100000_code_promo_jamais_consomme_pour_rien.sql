-- Un code promo ne doit pas s'user pour zéro ariary.
--
-- TAXIFOOD50 porte sur la livraison et n'est valable QU'UNE FOIS par client
-- (contrainte unique `promo_redemptions (code_id, user_id)`). La remise est
-- calculée par `remise_promo`, qui la plafonne à sa base : pour un restaurant
-- dont `delivery_fee` vaut 0, elle vaut donc 0.
--
-- Dans cet état, avant ce correctif :
--   * `verifier_code_promo` répondait « valide », et l'écran affichait
--     « Code TAXIFOOD50 appliqué — Tu économises 0 Ar » ;
--   * `create_order` insérait quand même la ligne `promo_redemptions`, brûlant
--     l'unique utilisation du client sans lui donner un ariary, et tamponnait la
--     commande d'un `promo_code` qui n'avait rien changé au total.
--
-- Le client perdait son code et n'avait aucun moyen de le récupérer : rien, dans
-- tout le projet, ne libère une utilisation consommée. Aucun restaurant n'est à
-- 0 Ar de livraison aujourd'hui, mais rien n'empêche d'en créer un depuis le
-- tableau de bord admin — et le même raisonnement vaudra pour un futur code
-- « sous_total » sur un panier trop petit.
--
-- Le total facturé est IDENTIQUE dans les deux cas (retrancher 0 ne change
-- rien) : ce correctif ne déplace pas un ariary, il empêche seulement de
-- détruire un droit.

-- 1. L'aperçu le dit franchement plutôt que d'annoncer une économie nulle.
create or replace function public.verifier_code_promo(
  p_code text,
  p_restaurant_id uuid,
  p_sous_total integer default 0
) returns jsonb
language plpgsql
security definer
...
-- 2. Et la commande, seule autorité sur le montant, ne consomme rien pour rien.
--
-- ⚠️ SIGNATURE INCHANGÉE, au type de paramètre près. `create or replace` sur une
-- signature différente AJOUTE une surcharge et PostgREST répond alors PGRST203 :
-- plus aucune commande ne passe. C'est arrivé le 2026-09-05.
