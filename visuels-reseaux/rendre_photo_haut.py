...
async def rendre(nom, photo, larg=1350, haut=1800, sortie=None, **kw):
    ...
    open(f'pd-{nom}.dc.html', 'w').write(
        g.visuel_photo_bloc_haut(photo=photo, larg=larg, haut=haut, **kw))
    ...
    async with async_playwright() as pw:
        ...
        v = await pg.evaluate("""() => {
          ...
          return {col: bx('ph-col'), logos: bx('ph-logos'),
    ...
    marges = all(v[c]['L'] >= M-2 and v[c]['R'] <= larg-M+2 for c in ('col','logos')) \
             and v['col']['T'] >= M-4 and v['logos']['B'] <= haut-M+2 and v['col']['B'] <= 480
    propre = res['col'][0] >= 0.90 and res['logos'][0] >= 0.90
    cadre  = v['w'] == larg and v['h'] == haut
    contr  = min(res['col'][1], res['logos'][1]) >= 4.50
    ok = marges and propre and cadre and contr

    print(f"{sortie}   fond mesure RGB {coul[0]:.0f} {coul[1]:.0f} {coul[2]:.0f}")
    print(f"  1 marges      bloc haut {v['col']['L']}..{v['col']['R']} / {M}..{larg-M}   "
          f"bas du bloc haut {v['col']['B']}/480   logos bas {v['logos']['B']}/{haut-M}   {'ok' if marges else 'NON'}")
    print(f"  2 texte sur FOND   haut {100*res['col'][0]:.0f} %   bas {100*res['logos'][0]:.0f} %   (mini 90 %)   {'ok' if propre else 'NON'}")
    print(f"  3 cadre       {v['w']} x {v['h']}                                  {'ok' if cadre else 'NON'}")
    print(f"  4 contraste   noir/fond  haut {res['col'][1]:.2f}:1   bas {res['logos'][1]:.2f}:1   (mini 4,50)   {'ok' if contr else 'NON'}")
