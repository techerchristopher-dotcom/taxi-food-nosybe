...
- **Facebook** — `react-native-fbsdk-next`, `LoginManager.logInWithPermissions()` puis `signInWithIdToken({provider:'facebook'})` avec le jeton d'accès classique (pas un JWT — Supabase interroge l'API Graph côté serveur avec l'App Secret). ⚠️ **`loginTrackingIOS: 'enabled'` est obligatoire en second argument** de `logInWithPermissions` : sans lui, le SDK bascule silencieusement en mode iOS « Limited Login » (repérable à l'écran par l'URL `limited.facebook.com`), qui renvoie un jeton d'une autre nature que Supabase ne sait pas valider — **le SDK réussit tout son échange, mais aucune identité n'est jamais créée côté Supabase, sans erreur claire**. Piège trouvé en confirmant via `auth.identities` qu'aucune ligne `provider='facebook'` n'existait, malgré un flux qui semblait aboutir. Côté Meta, la permission `email` doit être ajoutée dans **Use Cases → Authentication and Account Creation** (écran distinct de la config du produit Facebook Login — absent d'un prompt de setup qui ne couvrirait que ce dernier) ; « Allow users without an email » activé côté Supabase en filet de sécurité.
