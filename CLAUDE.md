...
- **Non fait (P2 futur)** : édition menu/horaires depuis l'app, filtrage livreur par zone (`couriers.zone`), stats. Notifications push : **faites** pour le client (voir plus bas) ; restent à faire pour le **restaurant** (nouvelle commande) et le **livreur** (course disponible) — la mécanique est en place, il n'y a qu'à élargir le trigger et les destinataires.
...
orders.status change  →  trigger orders_notify_status  →  pg_net
...
- **Trigger `orders_notify_status`** (`AFTER UPDATE OF status, picked_up_at ON orders`) : appelle l'Edge Function par `net.http_post` (pg_net, asynchrone — la commande n'attend jamais la notification). Il écoute **aussi `picked_up_at`** parce que la prise en charge par le livreur ne change pas le statut (`en_livraison` du début à la fin) alors que c'est le moment que le client attend le plus.
...
- **App** : `app/lib/push.ts` (enregistrement, ne lève jamais — une notification ratée ne doit pas empêcher de commander), écouteur de tap dans `_layout.tsx` qui ouvre `/order/[id]`, désenregistrement dans `signOut` **avant** `auth.signOut` (la RPC a besoin du jeton d'accès encore valide).
...
- ✅ **Vérifié de bout en bout côté serveur** (jetons de test, base restaurée après) : refus sans secret (403), envoi réel à Expo sur changement de statut, message de prise en charge sur `picked_up_at`, statut `recue` ignoré, suppression du jeton mort. Reste à voir sur un vrai téléphone au prochain build.
