...
- **Facebook** — `react-native-fbsdk-next` en mode **« Limited Login »** : `logInWithPermissions([...], 'limited', nonce)` puis `AuthenticationToken.getAuthenticationTokenIOS()`, dont le JWT part dans `signInWithIdToken({provider:'facebook', token, nonce})`.
